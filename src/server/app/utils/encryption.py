...
class Encryptor:
    """Utility class for symmetric encryption and decryption using Fernet (AES-128-CBC + HMAC)."""
    ...
    def _load_key() -> bytes:
        ...
        if not key:
            raise EnvironmentError('ENCRYPTION_KEY is not defined in environment variables.')
        return key.encode()
